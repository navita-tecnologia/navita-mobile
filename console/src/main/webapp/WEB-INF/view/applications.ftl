	...
	<@s.actionmessage/>
				...
				<div class="body">
					...
					<table class="gridContent">
						<tr>
							...
							<th width="100px" class="left">Mobile-ID</th>
							<th width="100px">Login nativo</th>
							...
							<tr class="active" valign="middle" <#if stat.odd>class="active"</#if>>
								...
								<td align="center" ><@s.if test="usingNativeLoginService">Sim</@s.if><@s.else>N&atilde;o</@s.else></td>
								<td align="center">
		...
		function removeApplication(nome,id){
			if(confirm('Tem certeza que quer remover a aplicacao '+nome+'?')){
				window.location='<@s.url action="applications!remove.action"/>?mobileApplication.id='+id	
			}
		}
		</script>
		</@t.template>
