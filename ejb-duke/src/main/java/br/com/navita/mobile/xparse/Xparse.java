...
public class Xparse {
  ...
  */
  ...
  static int count = 0;
  static NodeList index = new NodeList();
  ...
  Frag compile(Frag frag) {//NOPMD
	  ...
	  while(true)
	  {
  		...
  		if(TagStart != 0)
	  	{
	  		...
	  		if(TagStart == -1)
		  	{
			  	frag.ary.setElementAt(entity(frag.str), thisary, NodeList.Value);
				  frag.str = "";
  			}
	  		else
		  	{
			  	frag.ary.setElementAt(entity(substring(frag.str,0,TagStart)), thisary, NodeList.Value);
  ...
  Frag tagElement(Frag frag)
  {
	  ...
	  frag.ary.setElementAt(strip(name), thisary, NodeList.Name);
  	if(attribs.length() > 0)
	  {
		  frag.ary.setElementAt(attribution(attribs), thisary, NodeList.Attributes);
  	}
	  if(!empty)
  	{
	  	// !!!! important,
	  	...
	  	frag.ary.setElementAt(contents.ary, thisary, NodeList.Contents);
  ...
  Frag tagPI(Frag frag)
  {
  	...
  	frag.ary.setElementAt(val, thisary, NodeList.Value);
  	frag.str = substring(frag.str,close + 2, frag.str.length());
  	return frag;
  }
...
Hashtable attribution(String str)//NOPMD
  {
   ...
   */
  String prolog(String str)//NOPMD
  {
   ...
   */
  String normalize(String str)//NOPMD
  {
	  NodeList A = new NodeList();
   ...
   */
  String entity(String str)//NOPMD
  {
	  NodeList A = new NodeList();
